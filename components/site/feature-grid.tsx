...
const items = [
  {
    ...
    desc: "1-click AWS Secrets Manager sync injected natively into PTY without touching disk.",
  },
  {
    icon: CheckListIcon,
    title: "AI Pull Requests",
    desc: "1-click PR generation using gh CLI right from the source control panel.",
  },
  {
    icon: CommandIcon,
    title: "Tutor Mode",
    desc: "Cmd+Shift+L instantly summons AI to debug your failing terminal commands.",
  },
  {
    icon: CommandIcon,
    title: "Native Vibrancy",
    desc: "Gorgeous macOS native glassmorphism, acrylic blurs, and dynamic theming.",
  },
  {
    icon: Mic01Icon,
    title: "Voice input",
    ...
    <Section id="more">
      ...
      <div className="mt-14 grid grid-cols-1 gap-px overflow-hidden rounded-3xl border border-primary/20 bg-primary/10 sm:grid-cols-2 lg:grid-cols-4 shadow-[0_0_30px_color-mix(in_srgb,var(--primary)_10%,transparent)]">
        {items.map((it) => (
          <div
            key={it.title}
            className="group relative bg-background/80 p-8 backdrop-blur-md transition-all duration-300 hover:bg-primary/5 hover:z-10 hover:shadow-[0_0_30px_color-mix(in_srgb,var(--primary)_15%,transparent)]"
          >
